    ...
    public function showClassWiseResult(Request $request)
    {
        if (hasPermission('see_result')) {
            ...
            if ($request->resultType == "studentWise") {
                ...
                if ($checkTotalMark > 0) {
                    return  $markType = MarkType::where('institute_classes_id', $request->student_wise_class_id)
                        ->where('school_id', authUser()->id)->orderBy('id', 'Asc')->get();
                    // return  $markType = ResultSubjectCountableMark::where('institute_class_id', $request->student_wise_class_id)
                    //     ->where('school_id', authUser()->id)->orderBy('id', 'Asc')->get()->groupBy('subject_id');
                ...
                if ($request->result_type == 'all_student') {
                    ...
                    foreach ($studentResults as $key => $result) {
                        if (
                            !is_null($result->term) && !is_null($result->subject)
                            &&
                            (in_array($result->subject->group_id, [0, $my_group_id]) || ($optional_subject_code ? $optional_subject_code[0] : '') == $result->subject?->subject_code)
                        ) {
                    ...
                    $failed_students_id= Result::where('school_id', authUser()->id)->where('institute_class_id', $request->final_wise_class_id)->where('section_id', $section->section_id)->where('gpa','0')->pluck('student_id')->unique();
                      
                   $section_rank = Result::select('student_id', 'attend_days')->selectRaw("SUM(total) as finalTotal")
                        ->where('school_id', authUser()->id)
                        ->where('institute_class_id', $request->final_wise_class_id)
                        ->where('section_id', $section->section_id)
                        ->whereNotIn('student_id', $failed_students_id)
                        ->orderByDesc('finalTotal')
                        ->orderByDesc('attend_days')
                        ->groupBy('student_id', 'attend_days')
                        ->get();
