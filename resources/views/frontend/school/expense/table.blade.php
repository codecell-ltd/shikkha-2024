    ...
    <div class="row">
        <div class="col-xl-12">
            <div class="card-header py-3 bg-transparent">
                <div class="d-sm-flex align-items-center">
                    ...
                    <div class="ms-auto">
                        ...
                        <a href="{{route('expense.create', ['expenseFund' => $expenseFund])}}" class="btn btn-primary"><i class="bi bi-plus-square"></i> {{__('app.Add new expense')}}</a>
                        @endif
            ...
            <div class="card shadow">
                ...
                <div class="card-body table-responsive" id="expense_list">
                    <center>
                        @if(isset($searchmonth))
                        <p style="margin: 0px;padding: 0px; font-size:18px; color:black;">Month: @if ($searchmonth == 1) January
                        ...
                        @if (isset($enddate))
                        <p style="margin: 0px; padding: 0px; font-size:18px; color:black;"> Expense Between: {{ date('d-m-Y', strtotime($searchdate)) }} to {{ date('d-m-Y', strtotime($enddate)) }}</p>
                        @else
                        <p style="margin: 0px; padding: 0px; font-size:18px; color:black;"> Expense on: {{ date('d-m-Y', strtotime($searchdate)) }}</p>
                        @endif
                        @else
                        @endif
                        <h5><span class="align: center;" style=" color:black;">{{__('app.Total Expense')}}: {{number_format($sumFund)}}</span> <i class="fa-solid fa-bangladeshi-taka-sign"></i></h5>
                    </center>
                    @if(hasPermission("expense_show"))
                    <table id="example" class="table table-striped table-hover data-table">
                        <button type="button" class="btn btn-danger btn-sm mb-2" id="delete_expense" data-bs-toggle="modal" data-bs-target="#delete_all_records">
                            {{__('app.deleteall')}}
                        </button>
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            ...
                            <tr id="expence_ids{{$item->id}}">
                                <td id="action_table_td"><input type="checkbox" class="check_ids" name="ids" value="{{$item->id}}"></td>
                                <th scope="row">{{++$key}}</th>
                                <td style="white-space: nowrap;">{{date('d-m-Y',strtotime($item->datee))}}</td>
                                <td>{{$item->purpose}}</td>
                                ...
                                <td>{{$item->amount}}</td>
                                <td>{{$item->remark}}</td>
                                <td class="text-nowrap" id="action_table_td">
                                    ...
                                    <button class="btn btn-sm btn-primary" onclick="if(confirm('Are you sure? you are going to delete this record')){ location.replace('delete/{{$item->id}}'); }">
                                        {{__('app.Delete')}}
                                    </button>@endif
                                </td>
...
<script>
    function printDiv(printDiv) {
        ...
        $("#printable_content").html($("#expense_list").html());
