...
<body>
    ...
    <div class="page-break section-to-print" id="printDiv" style="background: white;">
        ...
        <div class="page-break">
            <div class="fixed-width">
                ...
                <div class="mid-content clear">
                    <div class="header-left text-right left" style="padding-right: 20px;margin-left:5px;margin-top:5px;">
                        @if (File::exists(public_path(authUser()->school_logo)) && !is_null(authUser()->school_logo))
                        <img src="{{asset(authUser()->school_logo)}}" alt="school logo" class="" width="80" style="width:100px; height:100px; margin-right:30px;">
                        @endif
                    </div>
                    <div class="header-right text-center left" style="margin-left: -137px;">
                        <h3 style="margin-bottom: 2px;"> {{ strtoupper(authUser()->school_name) }} </h3>
                        <p style="margin-bottom: 2px;"> <b>{{ authUser()->slogan != null ? '('.authUser()->slogan.')': "A School for Shikkha"}}</b> </p>
                        <p style="margin-bottom: 2px;"> {{ authUser()->address }} </p>
                        <h5>{{ $term->title }}</h5>
                    </div>
                </div>
                {{-- End School Info --}}

                <hr style="color:rgb(189, 2, 180);margint-top:5px">

                {{-- Student Info --}}
                <div class="row" style="margin-bottom: 5px;">
                    <div class="col-2">
                        @if(File::exists(public_path($studentResults->first()->user?->image)))
                        <img src="{{asset($studentResults->first()->user?->image)}}" class="" alt="student image" style="height: 80px; width: 60px; margin-left:5px;">
                        @else
                        <img src="{{asset('d/no-img.jpg')}}" class="" alt="student image" style="height: 80px; width: 60px;">
                        @endif
                    </div>
                    <div class="col-5">
                        <div class="student_info" style="font-size: 12px;">
                    ...
                    <div class="col-3">
                        <div class="student_info" style="font-size: 12px;padding-left:40px;">
                            <table>
                                <tbody>
                                    <tr>
                                        <td>{{__('app.class')}}</td>
                    ...
                    <div class="col-2">
                        <div class="performance">
                            <table class="bordered-table" style="font-size: 12px; border-color:black; margin-right:2px;">
                                <thead>
                                    <tr align="">
                ...
                {{-- <div class="mid-content " style="margin-top: 10px;margin-bottom:10px">
                    <div class="clear">
        ...
        <div class="header-bottom-right right ">
            ...
            <div class="right ">
                <div class="performance">
                    <table class="bordered-table" style="font-size: 12px; border-color:black; margin-right:5px;">
                        <thead>
                            <tr align="">
                                <th colspan="2">Performance In Class</th>
                            </tr>
    ...
    {{-- Result Table --}}
    <div class="mid-content" style="height: 478px;">
        <table class="bordered-table" style="font-size: 12px; width:99%;margin-bottom:20px;margin-left:5px;">
            <thead>
                <tr style="padding-left:0px; margin: 0px;">
                    <th width="400px;" class="text-nowrap">Subject Name</th>
                    <th scope="col" style="padding-left:1px; padding-right:2px; margin: 0px;">Full Mark</th>
                    <th scope="col" style="padding-left:1px; padding-right:2px; margin: 0px;">Pass Marks</th>
                    @foreach ($markType as $data)
                    @if ($data->mark_type == 'Class_Test')
                    <th style="padding-left:1px; padding-right:2px; margin: 0px;">CT</th>
                    @else
                    <th style="padding-left:1px; padding-right:2px; margin: 0px;">{{$data->mark_type}}</th>
                    @endif
                    @endforeach
                    <th scope="col" style="padding-left:1px; padding-right:2px; margin: 0px;">Total Marks</th>
                    <th scope="col" style="padding-left:1px; padding-right:2px; margin: 0px;">Grade</th>
                    <th scope="col" style="padding-left:1px; padding-right:2px; margin: 0px;">Grade Point</th>
                </tr>
            </thead>
            <tbody style="padding:0px; font-size: 12px;">
                @php
                $total = 0;
                $totalSubject = 1;
                $totalGpa = 0.000;
                ...
                <tr>
                    ...
                    <td style="padding: 0px; color:black;">
                        @if ($result->grade == 'F')
                        <!-- @if( $studentResults->first()->user?->roll_number ==5)
                        @dd($result)
                        @endif -->

                        @php
                        $isOptional = isOptional($subject_id, $studentResults->first()->user->id);
                        if($isOptional == 0)
                        { $fail = 1;}
                ...
                if ($totalMark < $pass_mark || $fail==1) { $resultStatus="Fail" ; } @endphp @endif @endforeach {{-- @dd($totalSubject); --}} {{-- @dd($totalGpa); --}} @php $optionalSubject=$studentResults->first()->user?->optional_subject;
                    ...
                    $addOptionalPoint = $addOptionalPoint < 0 ? 0 : $addOptionalPoint; $totalGpa=$totalGpa + $addOptionalPoint; if($totalSubject==0){ $totalSubject=1; } $averageGpa=number_format($totalGpa / $totalSubject, 2); $averageGpa=$averageGpa>= 5 ? 5.00 : $averageGpa;
                        } else {
                        $totalSubject -= 1;
                        if($totalSubject==0){
                        $totalSubject = 1;
                        ...
                        $averageGpa = $averageGpa >= 5 ? 5.00 : $averageGpa;
                        }

                        $grading_point = array(
                        'A+' => 5.00, 'A' => 4.00, 'A-' => 3.5, 'B' => 3.00, 'C' => 2.0, 'D' => 1.0, 'F' => 0
                        );
    ...
    <div class="mid-content clear" style="margin-bottom:30px">
        <div class="header-bottom-left left">
            <div class="mark-left">
                <table class="bordered-table" style="font-size: 12px;width:100%; margin-left: 5px;">
                    <thead>
                        <tr align="">
            ...
            <div class=" mark-right">
                <div class="top">
                    <table class="bordered-table" style="font-size: 12px;width:100%">
                        <tbody>
                            <tr class="">
                                <td class="" style="border-bottom-color: white;" style="padding:2px;">Total Working Days</td>
                                <td class="" style="padding:2px;">{{ getWorkingDays(authUser()->id, $studentResults->first()->user?->id, $term->id) }}</td>
                            </tr>
                            <tr class="">
                                <td class="" style="padding:2px;">Present</td>
                                <td class="" style="padding:2px;">{{ getPresentDays(authUser()->id, $studentResults->first()->user?->id, $term->id) }}</td>
                            </tr>
                            <tr class="">
                                <td class="" style="padding:2px;">Absent</td>
                                <td class="" style="padding:2px;">{{ getAbsentDays(authUser()->id, $studentResults->first()->user?->id, $term->id) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                    ...
                    <table class="bordered-table" style="font-size: 12px;width:100%">
                        <tbody>
                            <tr class="">
                                ...
                                <td class="" style="padding:2px;">{{ singleStudentRank($class_id, $term_id, $studentResults->first()->user?->id) }}</td>
                                {{-- <td class="" style="padding:2px;">{{(isset($resultStatus)) ? ' ' : $studentRank}}</td> --}}
                            </tr>
                            <tr class="">
                                <td class="" style="padding:2px;">Position in Section</td>
                                <td class="" style="padding:2px;">{{ sectionWiseSingleStudentRank($class_id, $section_id, $term_id, $studentResults->first()->user?->id) }}</td>
        ...
        <div class="header-bottom-right right" style="font-size: 12px;">

            <table class="bordered-table" style="width:99%;">
                <thead>
                    <tr>
