                    ...
                    @if (App\Models\AddonModel::where('feature_id', 11)->where('status', 1)->exists())
                        ...
                        @endif
                    @else
                        <li class="nav-item mb-2 text-center @if (App\Models\FeatureList::where('id', 11)->where('status', 0)->exists()) deasableFeature @endif"
                            data-bs-toggle="tooltip" data-bs-placement="right" title="{{ __('app.Student') }}">
                            <a href="javascript::"><button class="nav-link mb-0"
                                    data-bs-toggle="pill" data-bs-target="#pills-student" type="button"><img
                                        src="{{ asset('assets/nav-icons-white/student.svg') }}" alt="student"
                                        width="20"></button></a>
