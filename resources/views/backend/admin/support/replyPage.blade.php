        ...
        <div class="row">
            <section class="discussions" style="width: 35%; height: 600px; box-shadow: 0px 8px 10px rgba(0, 0, 0, 0.20);
                ...
                color: #515151;" onclick="loadData('{{ $data->id }}')">
                        ...
                        <p style="margin-left: 10px;">
                            {!! substr(strip_tags($data->subject), 0, 30) !!}</p>
                ...
                border-radius: 15px;">
                    ...
                    <a style="padding:2%" href="" onclick="if(confirm('Are you sure? You are going to delete this record')){ location.replace( '{{route('ticket.delete.admin',$data->id) }}' ); }"><i class="bi bi-trash"></i></a>
            ...
            <section class="chat" style="width: 65%;padding-left: 0px;
                ...
                color: #515151;">
                    ...
                    <div class="card-body" style="height:420px;overflow: scroll;"">
                        <div id="dataLoad">
                        </div>
                        </div>
...
<script>
    $(document).ready(function() {
        $('#myForm').submit(function(e) {
            ...
            $.ajax({
                url: "{{ route('ticket.reply.post.admin') }}",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
    ...
    function loadData(para) {
        $("#ticket_id").val(para);
        $.ajax({
            url: "{{ route('ticketmessage.show.admin', ':id') }}".replace(':id', para),
            type: 'GET',
            dataType: 'json',
            success: function(response) {
                ...
                response.forEach(function(item) {
                    ...
                    if (item.attachment == null) {
                        if (item.assign_id_admin == null) {
                            var createdAt = new Date(item.created_at);
                            var formattedDate = createdAt.toLocaleString();
                            html +=
                                '<div style="display: flex; align-items: flex-start; margin-bottom: 10px;">' +
                                '<img src="' + item.assign_user.school_logo + '"  style="width: 40px; height: 40px; border-radius: 50%; margin-right: 10px;">' +
                            ...
                            html +=
                                ...
                                '<img src="' + item.assign_admin.admin_logo + '"  alt="Profile Image" style="width: 40px; height: 40px; border-radius: 50%; margin-left: 10px;">' +
                                '</div>'
                        }
                    } else {
                        if (item.assign_id_admin == null) {
                            ...
                            if (item.message == null) {
                                html +=
                                    '<div>' +
                                    '<div style="display: flex; align-items: flex-start; margin-bottom: 10px;">' +
                                    '<img src="' + item.assign_user.school_logo + '"  alt="Profile Image" style="width: 40px; height: 40px; border-radius: 50%; margin-right: 10px;">' +
                            ...
                            if (item.message == null) {
                                html += '<div>' +
                                    ...
                                    '</div>' +
                                    '<div>' +
                                    '<div>' + '<img style="width:120px;margin-bottom:5px; height:100px; margin-left: 70%;" src=" ' + item.attachment + '">' + '</div>' +
                                    '</div>'
                            } else {
                                html += '<div>' +
                                    '<div style="display: flex; align-items: flex-start; margin-bottom: 10px;">' +
